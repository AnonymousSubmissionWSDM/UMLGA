"""
@Filename       : subgraph_data_processing.py
@Create Time    : 2022/11/5 22:12
@Author         : https://github.com/mims-harvard/G-Meta/blob/1bcc4f61997b94f795bbe597f2d991cd6f847b93/G-Meta/subgraph_data_processing.py
@Description    : 
...
class Subgraphs(Dataset):
        ...
        if self.task_setup == 'Disjoint':
            ...
            if self.link_pred_mode:
                ...
                for i, (k, v) in enumerate(dictGraphsLabels.items()):
                    # self.data_label[k] = []
                    for m, n in v.items():
                        self.data_label[relative_idx_map[k]].append(
                            n)  # [(graph 1)[(label1)[subgraph1, subgraph2, ...], (label2)[subgraph111, ...]], graph2: [[subgraph1, subgraph2, ...], [subgraph111, ...]] ]
    ...
    def create_batch_shared(self, batchsz):
        ...
        for b in range(batchsz):  # one loop generates one task
            ...
            selected_cls = np.array(
                list(range(len(data))))  # for multiple graph setting, we select cls_num * k_shot nodes
            np.random.shuffle(selected_cls)
    ...
    def create_batch_LinkPred(self, batchsz):
        ...
        for b in range(batchsz):  # one loop generates one task
            ...
            selected_cls_spt = np.array(
                list(range(len(data_spt))))  # for multiple graph setting, we select cls_num * k_shot nodes
            np.random.shuffle(selected_cls_spt)

            data_qry = self.data_label_qry[selected_graph]

            selected_cls_qry = np.array(
                list(range(len(data_qry))))  # for multiple graph setting, we select cls_num * k_shot nodes
            ...
            for cls in selected_cls_spt:
                ...
                np.random.shuffle(selected_subgraphs_idx)
                support_x.append(
                    np.array(data_spt[cls])[
                        selected_subgraphs_idx].tolist())  # get all subgraphs filename for current Dtrain
    ...
    def __getitem__(self, index):
        ...
        if self.link_pred_mode:
            info = [self.generate_subgraph_link_pred(self.G[int(item.split('_')[0])], int(item.split('_')[1]),
                                                     int(item.split('_')[2]), item)
